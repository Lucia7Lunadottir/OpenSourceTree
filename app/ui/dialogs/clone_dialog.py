...
class CloneDialog(QDialog):
    ...
    def _browse_dest(self):
        path = QFileDialog.getExistingDirectory(
            self, "Select Destination Directory", os.path.expanduser("~")
        )
        if path:
    ...
    def _on_clone(self):
        ...
        if not url:
            QMessageBox.warning(self, "Clone", "Please enter a repository URL.")
            return
        ...
        if not name:
            ...
            name = self._name_edit.placeholderText()
    ...
    def _on_clone_error(self, error: str):
        ...
        self._status_label.setText(lines[-1] if lines else "Clone failed")
